...
extern TimerEvent_t KeyHandleTimer;
extern TimerEvent_t FailLedTimer;
extern TimerEvent_t JoinLedTimer;

extern  uint8_t failClor;

LoRaMacPrimitives_t LoRaMacPrimitives;
LoRaMacCallback_t LoRaMacCallbacks;
MibRequestConfirm_t mibReq;

static void LoRaNetwork_Loop(void)
{
...
#error "Please define a region in the compiler options."
	...
	case DEVICE_STATE_SEND: {
			g_DeviceState = DEVICE_STATE_CYCLE;
		break;
	}
...
//			// Wake up through events
	...
	default: {
		g_DeviceState = DEVICE_STATE_INIT;
		break;
	}
	}
}
...
int main(void) {
  ...
  uint32_t bootCfg;
	...
	lorabutton_param_init();

	g_DeviceState = DEVICE_STATE_INIT;
	printf("Welcome to RAK_LB801\r\n");
	
	    //boot clear
    bootCfg = _get_boot_config();
    if( !(bootCfg & CONFIG_BOOT_E)) {
       bootCfg |= CONFIG_BOOT_E;
       _set_boot_config(bootCfg);
    }
